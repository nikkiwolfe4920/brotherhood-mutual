...
>
> **Ninth page, replaced — Global Engagement Command Center:** `/ExploreGod-Global` originally
> shipped as "Global Overview," a cross-region rollup of the same regional-coordinator data as
> `/ExploreGod-CRM-Dashboard` (region name, lead, missionary count, formation→discipleship rate).
> A later revision replaced it outright with a different product for a different role, built from
> a dedicated product brief for **Bronwyn**, a global engagement intelligence lead — not a bigger or
> more-filtered copy of either existing dashboard. Her job isn't "how many conversations happened,"
> it's "what is happening across global engagement, where are the problems or opportunities, and
> what needs investigation" — so the page leads with AI-surfaced findings and evidence, not a raw
> numbers dump, mirroring the brief's own framing: *Data → export → spreadsheet → manually inspect →
> report → coach*, becoming *Data → AI analysis → anomalies → investigation → evidence → coaching →
> report*. This is the same "different mental model, not more filters" principle the brief applies
> explicitly to Bronwyn vs. Vero (`/ExploreGod-CRM-Dashboard-2`) — two dashboard paradigms, not one
> dashboard with a role switch. Do not confuse the two by name: Vero's page is "Engagement Command
> Center"; Bronwyn's is "**Global** Engagement Command Center" — a global intelligence lens, not a
> per-missionary triage queue.
>
> It still loads `explore-god-dashboard.css` unchanged for the shell, tokens, and every shared
> component (`.egd-panel`, `.egd-stat`, `.egd-tabs`/`.egd-tab`, `.egd-avatar`, `.egd-ai-panel`), and
> keeps its own components in `design-system/explore-god-global.css` — same file, entirely new
> contents. Two small pieces first designed for `/ExploreGod-CRM-Dashboard-2`
> (`.egd-btn-cta`, the shared CTA button, and the `.egd-attn-row*` Needs Attention disclosure shell)
> are reused here too, but **duplicated** rather than cross-loading that page's stylesheet — every
> page here loads only the shared base plus its own file, never a sibling's, the same discipline
> `js/explore-god-global.js` already applies to small JS helpers (`escapeHtml`/`hashString`/
> `initials`/`renderSparkline`/`CTA_ARROW` — see `js/explore-god-dashboard-2.js` for the originals).
>
> The rail's "Global overview" icon/label was renamed **"Global Engagement Command Center"**
> everywhere it appears (all three sibling pages' rail nav) to match. Regions are real named regions
> this time (Africa, India, Indonesia, Philippines — matching the brief's own example data)
> rather than the old page's `#Region 1`–`8` placeholders, since the brief specifies them explicitly.
> See `COMPONENTS.md`'s "Global Engagement Command Center" entry for the full component/data
> breakdown, including the Conversation Review Workspace dialog and the Ask Engagement Intelligence
> panel.
>
> **Two shared-file fixes surfaced while building this page**, both general latent bugs rather than
> anything specific to Bronwyn's content, fixed once in `explore-god-dashboard.css` for every page
> that loads it: (1) `.egd-shell`'s grid (both its base rule and its `≤640px` override) used a bare
> `1fr` for the content column — shorthand for `minmax(auto, 1fr)`, whose automatic minimum is its
> content's min-content size, not `0` — which was invisible until this page put a wide
> `overflow-x: auto` table (the Regional Health matrix) inside `.egd-main` for the first time; fixed
> to `minmax(0, 1fr)` in both places, and `.egd-panel` itself picked up an explicit `min-width: 0` for
> the same reason (a flex item's automatic minimum is otherwise its content size). (2) the rail's
> `≤640px` "icons become a horizontal row" treatment never wrapped, and the rail has since grown to 8
> links across the three sibling pages — wide enough to force the whole page body to scroll
> horizontally at phone width, which this file's own responsive rule forbids; `.egd-rail__nav` now
> wraps (`flex-wrap: wrap`) instead.
>
